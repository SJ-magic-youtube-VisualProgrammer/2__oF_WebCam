	...
	********************/
	if(Cam_id == -2){
		...
		for(i = 0; i < Devices.size(); i++){
			...
			// if(Devices[i].deviceName != "FaceTime HDカメラ（内蔵）)" ){ // Device Nameは、変わってしまう可能性がある。「内蔵Camでないもので、最初に見つかったカメラ」と言う条件がこちら。
			if(Devices[i].deviceName.find("C920") != std::string::npos){
			// if(Devices[i].deviceName == "FaceTime HDカメラ（内蔵）" ){
				Cam_id = i;
				break;
			}
		}
...
void ofApp::draw(){
	...
	********************/
	...
	char buf[512];
	sprintf(buf, "Cam : %5.1f", Cam_FrameRate);
